...
int main(int argc,char** argv){
    ...
    cv::imshow("H",canaleshsv[0]);
    cv::imshow("S",canaleshsv[1]);
    cv::imshow("V",canaleshsv[2]);
    //cout<<canaleshsv[0];
    cv::Mat blurred;
    cv::GaussianBlur(canaleshsv[0],blurred,cv::Size(7,7),0);//filtro gaussiano con sus parametros
    cv::Mat blurredseg;
    cv::threshold(blurred,blurredseg,144,255,CV_THRESH_BINARY);
